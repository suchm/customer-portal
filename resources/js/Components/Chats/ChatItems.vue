...
<template>
    ...
    <ul role="list" class="overflow-hidden bg-white">
        ...
        >
            <div class="flex min-w-0 gap-x-4 items-center">
                <img class="h-12 w-12 flex-none rounded-full bg-gray-50" :src="chat.message[0].user.avatar" alt="">
                <div class="min-w-0 flex-auto">
                    <div class="flex justify-between">
                        <p class="text-sm font-semibold leading-6 text-gray-800">{{ chat.message[0].user.name }}</p>
                        <p class="text-xs leading-5 text-gray-500">{{ formatDistanceFromDate(chat.message[0].created_at) }}</p>
                    </div>
                    <h3>{{ chat.title }}</h3>
